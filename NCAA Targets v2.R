...
ScrapePlaybyPlay <- function(week, output.csv = FALSE, aws.send = TRUE){
gameids <- getGameIds(week)
...
if (aws.send){
  ...
  #Write tables
  print(paste("Adding week", str(week), "to AWS. Return value is TRUE if succeeded:", str(dbWriteTable(aws, name = "playbyplay", value = playbyplayout, row.names = FALSE, overwrite = FALSE, append = TRUE)) ))
